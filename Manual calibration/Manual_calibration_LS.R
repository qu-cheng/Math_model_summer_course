...
ui <- fluidPage(
  ...
  # App title ----
  titlePanel("Manual calibration with GOF"),
...
server <- function(input, output) {
  ...
  output$summary <- renderTable({
    ...
    SS <- sum((results$I - results$number_infected)^2, na.rm = TRUE)
    Likelihood <- sum(log(dpois(results$number_infected, results$I)), na.rm = TRUE)
    
    GOF <- data.frame(SS = SS, log_likelihood = Likelihood)
